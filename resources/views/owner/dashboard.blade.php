...
<body>
  ...
  <div class="main">
    <div class="topbar">
      <div style="display:flex;align-items:center;gap:12px"><button class="menu-toggle" onclick="document.querySelector('.sidebar').classList.toggle('hidden')">☰</button><h4>Selamat Datang, Owner</h4></div>
    ...
    <div class="content">
      <div class="dashboard">
        <div class="card big-card">
          ...
          <div class="stats">
            <div class="stat"><h2>Rp {{ number_format($ownerStats['totalRevenue'] ?? 0,0,',','.') }}</h2><p>Total Revenue</p></div>
            <div class="stat"><h2>{{ number_format($ownerStats['totalTransaksi'] ?? 0) }}</h2><p>Total Transaksi</p></div>
            <div class="stat"><h2>{{ $businessInsights['growthRate'] ?? 0 }}%</h2><p>Growth</p></div>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <ul class="list" style="margin:0;padding-left:18px">
            @forelse(($ownerStats['topMenu'] ?? []) as $m)
              <li>{{ $m->nama_menu }} — Terjual {{ $m->total_terjual }}</li>
            @empty
