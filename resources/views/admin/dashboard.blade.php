...
<head>
  ...
  <style>
    ...
    .big-card {
      background: url('https://images.unsplash.com/photo-1671034456366-77aff6bd3316?q=80&w=735&auto=format&fit=crop') no-repeat center/cover;
      position: relative;
      color: white;
      height: 240px;
      width: 100%;
...
<body>
  ...
  <div class="sidebar" id="sidebar">
    ...
    <a href="{{route('pelanggan.index')}}">Member</a>
    ...
    <div class="content">
      <div class="dashboard">
        <div class="card big-card">
          ...
          <div class="stats">
            <div class="stat">
              <h2>{{ number_format($totalPelanggan ?? 0) }}</h2>
              <p>Users</p>
            </div>
            <div class="stat">
              <h2>{{ number_format($totalPesananHariIni ?? 0) }}</h2>
              <p>Orders</p>
            </div>
            <div class="stat">
              <h2>{{ number_format($totalTransaksiHariIni ?? 0) }}</h2>
              <p>Tasks</p>
            </div>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <ul class="list">
            @forelse(($recentActivities ?? []) as $a)
              <li>{{ date('d M H:i', strtotime($a->created_at)) }} - {{ $a->nama_pelanggan }} (Meja {{ $a->id_meja }}) - Total Rp {{ number_format($a->total,0,',','.') }} {{ ($a->bayar ?? 0) > 0 ? '✔️' : '⏳' }}</li>
            @empty
    ...
    new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          data: @json($omzet7 ?? []),
          borderColor: '#1955aa',
          backgroundColor: 'rgba(25,85,170,0.2)',
          tension: 0.4,
          fill: true
        },{
          label: 'Pesanan',
